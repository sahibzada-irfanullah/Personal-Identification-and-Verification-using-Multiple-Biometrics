...
if(~isempty(cursor.Message) || isempty(cursor.Statement))
    ...
    database.DatabaseUtils.errorhandling('database:cursor:invalidInputCursor', cursor.Message);
    return;
end
...
if ~strcmpi(p.DataReturnFormat, 'cellarray') && isempty(str2num(p.NullNumberRead)) %#ok
    ...
    cursor.Message = m.getString;
    database.DatabaseUtils.errorhandling('database:cursor:invalidNullNumber', cursor.Message);
    return;
end
...
try
    ...
    cursor.Data = 0;
    database.DatabaseUtils.errorhandling(e.identifier, cursor.Message);
    return;
